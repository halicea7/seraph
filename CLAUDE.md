...
**Database:** `database.py` defines all 18 SQLAlchemy models. The cascade chain is:
...
**Schema migrations** are handled by `_migrate()` in `database.py` — a list of `ALTER TABLE` statements that are silently skipped if the column already exists. Add new columns here; never drop tables.
...
**Auth:** JWT Bearer tokens, 24 h expiry, signed with `SERAPH_SECRET_KEY` env var. No auth dependency is applied globally — individual routers opt in with `Depends(get_current_user)`. Most read endpoints are currently unauthenticated.
...
**Auth:** `AuthContext` stores the JWT in `localStorage` under `seraph_token` and attaches it as `Authorization: Bearer <token>` on `/api/v1/auth/me` at startup. The `api/client.ts` helper does **not** automatically attach the token — auth-required pages call `fetch` directly with the header.
