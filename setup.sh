...
if [ "$ready" = true ]; then
  ok "Seraph is up."
  echo ""
  printf "  ${GREEN}➜  Web UI / API:  ${URL}${RESET}\n"
  echo "     Open it in a browser and create your admin account on the First-Run screen."
  echo ""
  printf "  ${CYAN}SeraphElectron desktop app:${RESET}\n"
  ...
  echo "     $COMPOSE logs -f"
  echo "  Then open ${URL} (browser) or point the SeraphElectron app at it."
fi
